...
function getAppName(): string {
  try {
    const appJsonPath = path.resolve(PROJECT_ROOT, "..", "app", "app.json");
...
function configureExpoAndLanding(app: express.Application) {
  ...
  app.use((req: Request, res: Response, next: NextFunction) => {
    ...
    // In development, skip the landing page so `/` is proxied to Metro
  ...
  // In development, proxy unmatched non-API requests to the Expo Metro bundler
  // so the web app and API share the same origin (no cross-origin cookie issues).
  const metroPort = process.env.METRO_PORT || "8081";
  if (process.env.NODE_ENV !== "production") {
    app.use((req: Request, res: Response, next: NextFunction) => {
      // Let Express handle API routes — don't proxy them to Metro
      if (req.path.startsWith("/api")) {
        return next();
      }
      ...
      proxyReq.on("error", () => {
        if (!res.headersSent) {
          res.status(502).send("Metro bundler not running on port " + metroPort);
        }
      });
