import { appSettings, modelTrainingJobs } from "@swing-ai/shared/schema";
...
export async function getDriveMovementClassificationModelRuntimeSelection(actorUserId?: string | null): Promise<{
  ...
  const [modelPath, lstmModelPath] = await Promise.all([
    ensureLocalClassificationModelArtifact({
  ...
  return {
    ...
    env: {
      ...
      ...(lstmModelPath ? { SWING_AI_LSTM_MODEL_PATH: lstmModelPath } : {}),
